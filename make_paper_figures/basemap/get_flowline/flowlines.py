...
for i, site in enumerate(ds['site'].values):
    ...
    vx_interp = RegularGridInterpolator((x_grid, y_grid), dm['vx'].values.T, bounds_error=False, fill_value=np.nan)
    vy_interp = RegularGridInterpolator((x_grid, y_grid), dm['vy'].values.T, bounds_error=False, fill_value=np.nan)
