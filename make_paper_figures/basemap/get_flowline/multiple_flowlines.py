...
sites = [row['Site'] for row in site_data]
    ...
    # transform from 4326 to 3413
transformer = Transformer.from_crs("epsg:4326", "epsg:3413", always_xy = True)
site_x, site_y = transformer.transform(lons, lats)

for x_val, y_val, site in zip(site_x, site_y, sites):
    ...
    df.to_csv(f'flowline_{site}.csv', index=False)
