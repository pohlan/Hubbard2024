...
if __name__=='__main__':
	...
	if nbpartition >1:
		mesh_dir = '%s/partitioning.%s'%(mesh,str(nbpartition))
	...
	for kk in np.arange(nbpartition): 
		if nbpartition >1:
			nodes_file='%s/part.%s.nodes'%(mesh_dir,str(kk+1))
			elements_file='%s/part.%s.elements'%(mesh_dir,str(kk+1))
			bc_file='%s/part.%s.boundary'%(mesh_dir,str(kk+1))
			header_file='%s/part.%s.header'%(mesh_dir,str(kk+1))
		else:
		...
		if np.count_nonzero(NodeMoulin)==0:
			...
			for ii in np.arange(ms):
			    if (NodeMoulin[ii] >0):
			    	jj = jj + 1
			    	fid1.write('%g %g %g %g %g %g \n'%(nBC+jj,MaxBC+1,MinEIndex,0,101,NodeMoulin[ii]))
			fid1.close()
