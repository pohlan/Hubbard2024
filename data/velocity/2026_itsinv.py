...
# Module name starts with a digit, so it can't use plain `import` — load via
# importlib and bind to the short name the rest of this file uses.
import importlib
datacube_tools = importlib.import_module("2026_datacube_tools")


# Shared, read-only inputs for the parallel grid solve. Set once in the parent
# before the Pool is created; worker processes inherit it (copy-on-write on
# Linux 'fork'), so the big vx/vy stacks are NOT re-pickled per pixel.
_GRID_SHARED = {}


def _pool_solve_point(ij):
    """Worker: solve one pixel, reading the shared arrays from _GRID_SHARED."""
    s = _GRID_SHARED
    return solve_point(
        (ij, s["vx_mask"], s["vy_mask"], s["aq1"], s["aq2"], s["solu_dates"], s["lt"])
    )


def _pool_solve_stencil(ij):
    """Worker: solve one stencil, reading the shared arrays from _GRID_SHARED."""
    s = _GRID_SHARED
    return solve_stencil(
        (ij, s["vx_mask"], s["vy_mask"], s["aq1"], s["aq2"], s["solu_dates"],
         s["lt"], s["lx"], s["dx"])
    )
...
def grid_inversion(
    ...
    # Get xsub max and min dates
    date_max = xsub["acquisition_date_img2"].max().values
    date_min = xsub["acquisition_date_img1"].min().values
    ...
    # Make list of pixels to solve (indices only — the heavy arrays are shared
    # via _GRID_SHARED, not packed into every task).
    dx = 0 if lx == 0 else np.diff(xsub.x)[0]
    ij_list = [
        (i, j)
    ...
    # Publish the read-only inputs once; workers inherit them (fork) or get them
    # once per process, instead of re-pickling vx/vy for each of len(ij_list) tasks.
    global _GRID_SHARED
    _GRID_SHARED = dict(
        vx_mask=vx_mask, vy_mask=vy_mask, aq1=aq1, aq2=aq2,
        solu_dates=solu_dates, lt=lt, lx=lx, dx=dx,
    )
    worker = _pool_solve_point if lx == 0 else _pool_solve_stencil
    # Small chunksize keeps the tqdm bar smooth; per-task overhead is tiny now
    # that the heavy arrays are shared (only an (i, j) tuple crosses per task).
    chunksize = 4
    ...
    try:
        if ncpu == 1:
            cells = list(
                tqdm.tqdm(map(worker, ij_list), total=len(ij_list), disable=not pbar)
            )
        else:
            with multiprocessing.Pool(ncpu) as p:
                cells = list(
                    tqdm.tqdm(
                        p.imap(worker, ij_list, chunksize=chunksize),
                        total=len(ij_list),
                        disable=not pbar,
                    )
                )
